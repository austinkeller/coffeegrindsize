...
# Second version (seems better on Lido and Forte images)
...
# Third version (seems much better on Lido and Forte images)
def_maxcost = 0.35
...
# Roundness is defined as the ratio of cluster to all pixels in the smallest
# circle that encompasses all pixels of a cluster
def_min_roundness = 0
...
class coffeegrindsize_GUI:
    ...
    def __init__(self, root):
        ...
        # Remember the root object for the full user interface so that the
        # methods of coffeegrindsize_GUI can refer to it
        self.master = root
        ...
        # Size in pixels of the canvas where the pictures and figures will be
        # displayed
        ...
        # === Initialize the main frame that will contain option buttons and
        # settings and the image ===
        self.container_options = Frame(self.master, width=720)
        ...
        self.frame_options.grid(
            row=0, column=0, sticky="nsew", columnspan=1, rowspan=25
        )
        ...
        self.simple_frame_options.grid(
            row=0, column=0, sticky="nsew", columnspan=1, rowspan=25
        )
        ...
        # Comment on March 3, 2019: To add support for simplified output, use
        # argument simple_data_entry=simple_data_entry
        (
            self.pixel_length_var,
            self.pixel_length_id,
            self.simple_pixel_length_id,
        ) = self.label_entry(
            def_pix_len,
        ...
        (
            self.physical_length_var,
            self.physical_length_id,
            self.simple_physical_length_id,
        ) = self.label_entry(
            reference_objects_dict["Custom"],
            "Reference Physical Size:",
            "mm",
            entry_id=True,
            event_on_entry="update_pixel_scale",
            clear_on_click=True,
        )
        ...
        self.reference_object = self.dropdown_entry(
            "Reference Object:",
            list(reference_objects_dict.keys()),
            self.change_reference_object,
        )

        # Physical size of one pixel in the coffee grounds picture
        # For now this needs to be input manually
        self.pixel_scale_var = self.label_entry(
            def_pixel_scale,
            "Pixel Scale:",
            "pix/mm",
            clear_on_click=True,
            event_on_entry="update_statistics",
        )
        ...
        # Value of fractional threshold in units of flux in the blue channel
        # of the image
        self.threshold_var = self.label_entry(
            def_threshold,
            "Threshold:",
            "%",
            advanced=True,
        ...
        # Maximum cluster diameter that should be considered a valid coffee
        # particle
        self.max_cluster_axis_var = self.label_entry(
        ...
        # Minumum cluster surface that should be considered a valid coffee
        # particle
        self.min_surface_var = self.label_entry(
            def_min_surface,
            "Minimum Cluster Surface:",
            "pix²",
            advanced=True,
            clear_on_click=True,
        )

        # Minimum cluster roundness that should be considered a valid coffee
        # particle.
        # Roundess is defined between 0 and 1 where 1 is a perfect circle. It
        # represents the fraction of thresholded pixels inside the smallest
        # circle that encompasses the farthest thresholded pixels in one
        # cluster
        self.min_roundness_var = self.label_entry(
            def_min_roundness,
            "Minimum Roundness:",
            "",
            advanced=True,
        ...
        # Threshold to select pixels dark enough to serve as a reference in
        # the cost function in the cluster breakup step
        if self.display_advanced_options is True:
            self.reference_threshold_var = self.label_entry(
                def_reference_threshold, "Ref. Threshold:", "", advanced=True
            )
        else:
        ...
        if self.display_advanced_options is True:
            self.maxcost_var = self.label_entry(
                def_maxcost, "Max. Cost:", "", advanced=True
            )
        else:
        ...
        quick_checkbox = Checkbutton(
            self.frame_options,
            text="Quick & Approximate",
        ...
        self.hist_choices = [
            ...
            "Mass vs Surface",
            "Mass vs Volume",
            "Available mass vs Diameter",
            "Available mass vs Surface",
            "Available mass vs Volume",
            "Surface vs Diameter",
            "Surface vs Surface",
            "Surface vs Volume",
        ]
        ...
        self.histogram_type = self.dropdown_entry(
            "Histogram Options:",
            self.hist_choices,
            self.change_histogram_type,
            default_choice_index=self.default_histogram_choice,
        )
        ...
        self.legend_type = self.dropdown_entry(
            "Label Position:", self.legend_choices, self.change_histogram_type
        )

        # Label for the data
        self.data_label_var = self.label_entry(
            "Current Data",
            "Data Label:",
            "",
            columnspan=2,
            width=self.width_entries * 3,
            event_on_enter="create_histogram",
        )

        # Label for the comparison data
        (
            self.comparison_data_label_var,
            self.comparison_data_label_id,
            self.simple_comparison_data_label_id,
        ) = self.label_entry(
            "Comparison Data",
            "Comparison Label:",
            "",
        ...
        xaxis_auto_checkbox = Checkbutton(
            self.frame_options,
            text="Automated X axis | ",
            variable=self.xaxis_auto_var,
            command=self.xaxis_auto_event,
        )
        xaxis_auto_checkbox.grid(row=self.options_row, columnspan=1, sticky=E, column=0)

        # X axis range for the histogram figure
        self.xmin_var, self.xmin_var_id = self.label_entry(
            def_min_x_axis,
            "Min. X Axis:",
            "",
            entry_id=True,
        ...
        self.xlog_var.set(1)
        xlog_checkbox = Checkbutton(
            self.frame_options,
            text="Log X axis | ",
            variable=self.xlog_var,
            command=self.xlog_event,
        )
        xlog_checkbox.grid(row=self.options_row, columnspan=1, sticky=E, column=0)

        self.xmax_var, self.xmax_var_id = self.label_entry(
            def_max_x_axis,
            "Max. X Axis:",
            "",
            entry_id=True,
        ...
        nbins_auto_checkbox = Checkbutton(
            self.frame_options,
            text="Automated bins | ",
            variable=self.nbins_auto_var,
            command=self.nbins_auto_event,
        )
        ...
        # X axis range for the histogram figure
        self.nbins_var, self.nbins_var_id = self.label_entry(
            10,
            "Num. bins:",
            "",
            entry_id=True,
        ...
        self.output_dir_var, self.output_dir_label_id = self.label_entry(
            self.output_dir,
            "",
            "",
            entry_id=True,
        ...
        self.session_name_var = self.label_entry(
            def_session_name,
            "Base of File Names:",
            "",
            columnspan=2,
        ...
        self.display_type = self.dropdown_entry(
            "Display Type:", choices, self.change_display_type
        )
        ...
        self.reset_zoom_button = Button(
            self.frame_options, text="Reset View", command=self.reset_zoom
        )
        self.reset_zoom_button.grid(
            row=self.options_row, column=2, columnspan=1, sticky=W
        )
        self.options_row += 1

        # Simplified versions of zoom buttons
        self.simple_zoom_in_button = Button(
            self.simple_frame_options,
            text="Zoom In",
            command=self.zoom_in_button,
        )
        self.simple_zoom_in_button.grid(
            row=self.simple_options_row, column=0, columnspan=1, sticky=E
        )

        self.simple_zoom_out_button = Button(
            self.simple_frame_options,
            text="Zoom Out",
            command=self.zoom_out_button,
        )
        self.simple_zoom_out_button.grid(
            row=self.simple_options_row, column=1, columnspan=1, sticky=W
        )

        self.simple_reset_zoom_button = Button(
            self.simple_frame_options,
            text="Reset View",
            command=self.reset_zoom,
        )
        self.simple_reset_zoom_button.grid(
            row=self.simple_options_row, column=2, columnspan=1, sticky=W
        )
        self.simple_options_row += 1

        # Button for resetting all options to default
        reset_params_button = Button(
            self.frame_options,
        ...
        simple_reset_params_button = Button(
            self.simple_frame_options,
            text="Reset All Parameters",
            command=self.reset_status,
        )
        simple_reset_params_button.grid(
            row=self.simple_options_row, column=1, columnspan=2, sticky=E
        )
        ...
        self.frame_stats = Frame(
            self.container_options, bg=frame_stats_bg, padx=2, pady=10
        )
        self.frame_stats.grid(row=0, column=1, sticky="new", rowspan=1)

        title_label = Label(
            self.frame_stats,
            text="Properties of the Particle Distribution:",
        ...
        diam_average_entry = Label(
            self.frame_stats,
            textvariable=self.diam_average_var,
            width=stats_entry_width,
            bg=frame_stats_bg,
        )
        ...
        diam_stddev_label = Label(
            self.frame_stats,
            text="Scatter in Diameter:",
            bg=frame_stats_bg,
            font="Helvetica 14 bold",
        )
        diam_stddev_label.grid(row=stats_row, sticky=E, column=stats_column)
        diam_stddev_entry = Label(
            self.frame_stats,
            textvariable=self.diam_stddev_var,
            width=stats_entry_width,
            bg=frame_stats_bg,
        )
        ...
        surf_average_label = Label(
            self.frame_stats,
            text="Average Surface:",
            bg=frame_stats_bg,
            font="Helvetica 14 bold",
        )
        surf_average_label.grid(row=stats_row, sticky=E, column=stats_column)
        surf_average_entry = Label(
            self.frame_stats,
            textvariable=self.surf_average_var,
            width=stats_entry_width,
            bg=frame_stats_bg,
        )
        ...
        surf_stddev_label = Label(
            self.frame_stats,
            text="Scatter in Surface:",
            bg=frame_stats_bg,
            font="Helvetica 14 bold",
        )
        surf_stddev_label.grid(row=stats_row, sticky=E, column=stats_column)
        surf_stddev_entry = Label(
            self.frame_stats,
            textvariable=self.surf_stddev_var,
            width=stats_entry_width,
            bg=frame_stats_bg,
        )
        ...
        open_image_button = Button(
            toolbar,
            text="Open Image",
            command=lambda: self.open_image(None),
            highlightbackground=toolbar_bg,
        )
        open_image_button.pack(
            side=LEFT, padx=self.toolbar_padx, pady=self.toolbar_pady
        )

        # Button to select a reference object
        ref_obj_button = Button(
            toolbar,
            text="Select Reference Object",
            command=lambda: self.select_reference_object_mouse(None),
            highlightbackground=toolbar_bg,
        )
        ...
        region_button = Button(
            ...
            command=lambda: self.select_region(None),
            highlightbackground=toolbar_bg,
        )
        region_button.pack(side=LEFT, padx=self.toolbar_padx, pady=self.toolbar_pady)

        # Button to apply image threshold
        threshold_image_button = Button(
            toolbar,
            text="Threshold Image",
            command=lambda: self.threshold_image(None),
            highlightbackground=toolbar_bg,
        )
        threshold_image_button.pack(
            side=LEFT, padx=self.toolbar_padx, pady=self.toolbar_pady
        )
        ...
        psd_button = Button(
            toolbar,
            text="Launch Particle Detection",
            command=lambda: self.launch_psd(None),
            highlightbackground=toolbar_bg,
        )
        ...
        erase_button = Button(
            toolbar,
            text="Erase Clusters",
            command=lambda: self.erase_clusters(None),
            highlightbackground=toolbar_bg,
        )
        ...
        histogram_button = Button(
            ...
            command=lambda: self.create_histogram(None),
            highlightbackground=toolbar_bg,
        )
        ...
        blog_button = Button(
            toolbar,
            text="Read Coffee Ad Astra Blog",
            command=self.blog_goto,
            highlightbackground=toolbar_bg,
        )
        blog_button.pack(side=RIGHT, padx=self.toolbar_padx, pady=self.toolbar_pady)

        # Downsample button
        downsample_button = Button(
            toolbar2,
            text="Reduce Image Quality",
            command=self.downsample_image,
            highlightbackground=toolbar_bg,
        )
        downsample_button.pack(
            side=LEFT, padx=self.toolbar_padx, pady=self.toolbar_pady
        )
        ...
        load_data_button = Button(
            toolbar2,
            text="Load Data",
            command=lambda: self.load_data(None),
            highlightbackground=toolbar_bg,
        )
        load_data_button.pack(side=LEFT, padx=self.toolbar_padx, pady=self.toolbar_pady)

        # Button to load comparison data from disk
        load_comparison_data_button = Button(
            toolbar2,
            text="Load Comparison Data",
            command=lambda: self.load_comparison_data(None),
            highlightbackground=toolbar_bg,
        )
        load_comparison_data_button.pack(
            side=LEFT, padx=self.toolbar_padx, pady=self.toolbar_pady
        )

        # Button to flush comparison data
        flush_comparison_data_button = Button(
            toolbar2,
            text="Flush Comparison Data",
            command=self.flush_comparison_data,
            highlightbackground=toolbar_bg,
        )
        flush_comparison_data_button.pack(
            side=LEFT, padx=self.toolbar_padx, pady=self.toolbar_pady
        )
        ...
        subMenu.add_command(
            label="Open Image...", command=lambda: self.open_image(None)
        )
        subMenu.add_separator()

        # Add an option to downsample images
        subMenu.add_command(
            label="Reduce Image Quality...", command=self.downsample_image
        )
        # subMenu.add_command(
        ...
        subMenu.add_command(
            label="Turn Expert Mode On/Off...", command=self.toggle_expert_mode
        )
        subMenu.add_separator()
    ...
    def select_reference_object_mouse(self, event):
        ...
        if self.img_source is None:
            ...
            self.status_var.set(
                "Original Image not Loaded Yet... Use Open Image Button..."
            )
        ...
        # Update the user interface status
        self.status_var.set(
            "Use your mouse to click on one side of the reference object..."
            " You will then need to click again to set up a measuring line..."
        )
    ...
    def select_region(self, event):
        ...
        # Update the user interface status
        self.status_var.set(
            "Click on the image to draw a polygon enclosing the coffee grounds, then press q..."
        )
    ...
    def quit_region_select(self, event):
        ...
        if self.mouse_click_mode == "SELECT_REGION":
            ...
            if polysize < 3:
                ...
                # Update the user interface status
                self.status_var.set(
                    "Analysis Region Did Not Have Enough Corners (at least 3 are needed)..."
                )

            else:

                # Add last polygon corner
                self.polygon_alpha = np.append(
                    self.polygon_alpha, self.polygon_alpha[0]
                )
    ...
    def change_reference_object(self, *args):
        ...
        self.physical_length_var.set(
            reference_objects_dict[self.reference_object.get()]
        )
        ...
        self.status_var.set(
            "Reference Object set to " + self.reference_object.get() + "..."
        )
    ...
    def update_pixel_scale(self):
        ...
        try:
            pixel_scale = float(self.pixel_length_var.get()) / float(
                self.physical_length_var.get()
            )
            # Make it a string
    ...
    def change_display_type(self, *args):
        ...
        if self.display_type.get() == threshold_image_display_name:
            if self.img_threshold is None:
                ...
                # Update the user interface status
                self.status_var.set(
                    "Thresholded Image not Available Yet... Use Threshold Image Button..."
                )
        ...
        if self.display_type.get() == outlines_image_display_name:
            if self.img_clusters is None:
                ...
                # Update the user interface status
                self.status_var.set(
                    "Cluster Outlines Image not Available Yet... Use Launch Particle Detection Button..."
                )
        ...
        if self.display_type.get() == histogram_image_display_name:
            if self.img_histogram is None:
                ...
                # Update the user interface status
                self.status_var.set(
                    "Histogram Figure not Available Yet... Use Create Histogram Figure Button..."
                )
    ...
    def dropdown_entry(
        ...
        # Also create simple version if needed
        if advanced is False:
            simple_dropdown_menu = OptionMenu(
                self.simple_frame_options, data_var, *choices
            )
    ...
    def label_entry(
        ...
        if text != "":
            ...
            if advanced is False:
                ...
                simple_data_label.grid(
                    row=self.simple_options_row, sticky=E, column=addcol
                )
        ...
        if event_on_entry is not None:
            ...
            data_var.trace(
                "w",
                lambda name, index, mode, data_var=data_var: function_trigger(),
            )
        ...
        if advanced is False:
            ...
            if clear_on_click is True:
                simple_data_entry.bind(
                    "<Button-1>",
                    lambda event: self.clear_entry(simple_data_entry),
                )
        ...
        if units_text != "":
            ...
            # Also display simplified version if required
            if advanced is False:
                simple_data_label_units = Label(
                    self.simple_frame_options, text=units_text
                )
    ...
    def label_title(self, text, advanced=False):
        ...
        title_label.grid(
            row=self.options_row, sticky=W, padx=self.title_padx, columnspan=2
        )
        self.options_row += 1
        ...
        if self.polygon_alpha is not None:
            ...
            if npoly > 1:
                ...
                for i in range(npoly - 1):
                    ...
                    line_x_start = (
                        self.polygon_alpha[i] - orig_nx / 2
                    ) * self.scale + image_x
                    line_y_start = (
                        self.polygon_beta[i] - orig_ny / 2
                    ) * self.scale + image_y
                    line_x_end = (
                        self.polygon_alpha[i + 1] - orig_nx / 2
                    ) * self.scale + image_x
                    line_y_end = (
                        self.polygon_beta[i + 1] - orig_ny / 2
                    ) * self.scale + image_y
                    ...
                    # Redraw line
                    _ = self.image_canvas.create_line(
                        line_x_start,
                        line_y_start,
                        line_x_end,
                        line_y_end,
    ...
    def release_mouse(self, event):
        ...
        if self.mouse_click_mode == "SELECT_REFERENCE_OBJECT":
            ...
            self.status_var.set(
                "The length of the the reference object was set to "
                + self.pixel_length_var.get()
                + " pixels... Now select its physical size with the library in the Reference Object dropdown menu, or with a custom Reference Physical Size..."
            )
        ...
        if self.mouse_click_mode == "SELECT_REFERENCE_OBJECT_READY":
            ...
            # Refresh the user interface status
            self.status_var.set(
                "Drag the red line across the reference object then click again... Be careful to exclude shadows, and make sure that the Angle of Reference Line entry is satisfactory to you..."
            )
    ...
    def move_start(self, event):
        ...
        if self.erase_clusters_mode is True:
            ...
            dist = np.sqrt(
                (self.clusters_xmean - mouse_beta) ** 2
                + (self.clusters_ymean - mouse_alpha) ** 2
            )
    ...
    def line_move(self, event):
        ...
        # Redraw line
        self.line_obj = self.image_canvas.create_line(
            self.linex_start, self.liney_start, cur_x, cur_y, fill="red"
        )

        # Update length of line in pixels
        line_length = (
            np.sqrt((cur_x - self.linex_start) ** 2 + (cur_y - self.liney_start) ** 2)
            / self.scale
        )
        ...
        line_angle = (
            180.0
            / np.pi
            * np.arctan2((cur_y - self.liney_start), (cur_x - self.linex_start))
        )
    ...
    def eraser_circle_refresh(self, event):
        ...
        self.erasemode_current_circle = self.image_canvas.create_oval(
    ...
    def motion(self, event, only_refresh_display=False):
        ...
        if self.mouse_click_mode == "SELECT_REGION":
            ...
            if self.polygon_alpha is not None:
                ...
                line_x_start = (
                    self.polygon_alpha[-1] - orig_nx / 2
                ) * self.scale + image_x
                line_y_start = (
                    self.polygon_beta[-1] - orig_ny / 2
                ) * self.scale + image_y
                ...
                # Redraw line
                self.selreg_current_line = self.image_canvas.create_line(
                    line_x_start,
                    line_y_start,
                    line_x_end,
                    line_y_end,
    ...
    def select_output_dir(self):
        ...
        self.output_dir = filedialog.askdirectory(
            initialdir=self.output_dir, title="Select an output directory"
        )
    ...
    def downsample_image(self):
        ...
        # Resize image
        self.img_source = self.img_source.resize(
            (
                int(float(self.img.size[0]) / 2),
                int(float(self.img.size[1]) / 2),
            ),
            Image.ANTIALIAS,
        )
        self.img = self.img_source
        ...
        # Update the user interface status
        self.status_var.set(
            "The Image was Downsampled by a Factor two..."
            f"Current Size is ({str(self.img.size[0])}, {str(self.img.size[1])})"
        )
    ...
    def threshold_image(self, event):
        ...
        try:
            ...
            # Update the user interface status
            self.status_var.set(
                "Some Options in the User Interface are Invalid Numbers..."
            )
        ...
        if self.polygon_alpha is not None:
            ...
            if np.max(contained) is False:
                ...
                # Refresh the user interface status
                self.status_var.set(
                    "No Image Pixels were Located Inside of the Analysis Region"
                )
            ...
            self.background_median = np.median(
                self.imdata.flatten()[np.where(contained)]
            )
        else:
            self.background_median = np.median(self.imdata)

        # Create a mask for thresholded pixels
        self.mask_threshold = np.where(
            self.imdata < self.background_median * threshold / 100
        )
        ...
        if self.polygon_alpha is not None:
            ...
            if np.max(contained) is False:
                ...
                # Refresh the user interface status
                self.status_var.set(
                    "No Thresholded Pixels were Located Inside of the Analysis Region"
                )
            ...
            self.mask_threshold = (
                self.mask_threshold[0][np.where(contained)[0]],
                self.mask_threshold[1][np.where(contained)[0]],
            )

            # Keep a list of edge pixels
            self.mask_threshold_edge = self.points_along_polygon(
                self.mask_threshold[1].astype(float),
                self.mask_threshold[0].astype(float),
                self.polygon_alpha,
                self.polygon_beta,
            )
        ...
        self.status_var.set(
            "Image thresholded: "
            + thresholded_fraction_str
            + "% of all pixels were thresholded"
        )
    ...
    def points_along_polygon(self, X_points, Y_points, X_polygon, Y_polygon):
        ...
        for li in range(nlines):
            ...
            ipath = np.where(
                (ddline <= np.sqrt(2.0) * 1.01) & (dd1 <= dd12) & (dd2 <= dd12)
            )
    ...
    def attainable_mass_simulate(self, volumes):
        ...
        unreachable_volumes[iboulders[0]] = (
            4.0 / 3.0 * np.pi * (radii[iboulders[0]] - depth_limit) ** 3
        )
    ...
    def ey_simulate(self, surfaces):
        ...
        extractions = (
            extraction_speed / (k_reference + extraction_speed) * extraction_limit
        )
    ...
    def launch_psd(self, event):
        ...
        if self.mask_threshold is None:
            ...
            # Update the user interface status
            self.status_var.set(
                "Image not Thresholded Yet... Use Threshold Image Button..."
            )
        ...
        mask_threshold_sorted = (
            self.mask_threshold[0][sort_indices],
            self.mask_threshold[1][sort_indices],
        )
        ...
        for i in range(nmask):
            ...
            if i % 10 == 0:
                ...
                self.status_var.set(
                    "Iteration #"
                    + str(i)
                    + "; Fraction of thresholded pixels that were analyzed: "
                    + frac_counted_str
                    + " %"
                )
                self.master.update()
            ...
            dopen2 = (X_mask[icurrent] - X_mask[iopen]) ** 2 + (
                Y_mask[icurrent] - Y_mask[iopen]
            ) ** 2
            ...
            if self.quick_var.get() == 1:
                ...
                dcurrent2 = (X_mask[iclust] - X_mask[icurrent]) ** 2 + (
                    Y_mask[iclust] - Y_mask[icurrent]
                ) ** 2
                ...
                if icurrent_in_clust[0].size != 1:
                    raise ValueError(
                        "The starting pixel was not found in a cluster. This should never happen !"
                    )

                # Create a cost function along the positions of this cluster for pixel rejection
                cost = (
                    imdata_mask[iclust] - imdata_mask[icurrent]
                ) ** 2 / self.background_median ** 2
                ...
                for ci in range(iclust.size):
                    ...
                    idark_in_list = np.where(
                        imdata_mask[iclust[iclust_filtered]]
                        <= (
                            (self.background_median - imdata_mask[icurrent])
                            * reference_threshold
                            + imdata_mask[icurrent]
                        )
                    )
                    ...
                    if idark.size == 0:
                        raise ValueError(
                            "There should be at least one dark pixel in the cluster, this should never happen !"
                        )

                    # Pick the nearest reference dark pixel
                    if idark.size > 1:
                        dist2 = (X_mask[idark] - X_mask[iclust[ci]]) ** 2 + (
                            Y_mask[idark] - Y_mask[iclust[ci]]
                        ) ** 2
                    ...
                    if iclust[ci] == idark:
                        ...
                        # Select all pixels within the path of the line
                        ipath = np.where(
                            (ddline <= np.sqrt(2.0)) & (dd1 <= dd12) & (dd2 <= dd12)
                        )
            ...
            surface_multiplier = (
                self.background_median - zlist.min()
            ...
            clusteri_data = {
                ...
                "XMEAN": xmean,
                "YMEAN": ymean,
                "XSTART": X_mask[icurrent].astype(int),
                "YSTART": Y_mask[icurrent].astype(int),
                "ZLIST": zlist,
                "ICLUST_FILTERED": iclust[iclust_filtered],
                "ICLUST": iclust,
                "MAXCOST_ALONG_PATH": maxcost_along_path,
                "COST": cost,
            }
        ...
        self.status_var.set(
            "Particle Detection Analysis Done ! Creating Cluster Map Image..."
        )
        self.master.update()
        ...
        # Refresh the user interface status
        self.status_var.set(
            "Particle Detection Analysis Done ! Cluster Map Image is Now Displayed..."
        )
    ...
    def refresh_cluster_data(self):
        ...
        for i in range(self.nclusters):
            ...
            for j in range(xlist.size):
                ...
                # Count neighbors
                ineigh = np.where(
                    (np.abs(xlist - xlist[j]) <= 1) & (np.abs(ylist - ylist[j]) <= 1)
                )
                ...
                cluster_map_display[xlist[j], ylist[j], 0] = 255
                cluster_map_display[xlist[j], ylist[j], 1] = 0
                cluster_map_display[xlist[j], ylist[j], 2] = 0
    ...
    def quick_cluster(self, xlist, ylist, xstart, ystart):
        ...
        for i in range(xlist.size):
            ...
            isel = np.where(
                (np.abs(xlist_decay - xcheck[0]) + np.abs(ylist_decay - ycheck[0]))
                <= 1.001
            )
    ...
    def psd_hist_from_data(
        ...
        if "vs Diameter" in self.histogram_type.get():
            data = (
                2
                * np.sqrt(source.clusters_long_axis * source.clusters_short_axis)
                / pixel_scale
            )
        ...
        if "Available mass vs" in self.histogram_type.get():
            data_weights = self.attainable_mass_simulate(
                source.clusters_volume / pixel_scale ** 3
            )
            self.ylabel = "Fraction of Available Mass"

        if ("Extracted mass vs" in self.histogram_type.get()) or (
            "Extraction Yield Distribution" in self.histogram_type.get()
        ):
            reachable_vol = self.attainable_mass_simulate(
                source.clusters_volume / pixel_scale ** 3
            )
        ...
        if bins_input is None:
            ...
            if self.nbins_auto_var.get() == 1:
                ...
                if self.xlog_var.get() == 1:
                    nbins = int(
                        np.ceil(
                            np.log10(float(histrange[1]) - np.log10(histrange[0]))
                            / float(default_log_binsize * default_bin_inflate)
                        )
                    )
                else:
                    nbins = int(
                        np.ceil(
                            float(histrange[1] - histrange[0])
                            / float(default_binsize * default_bin_inflate)
                        )
                    )
            else:
                try:
            ...
            if self.xlog_var.get() == 1:
                bins_input = np.logspace(
                    np.log10(histrange[0]), np.log10(histrange[1]), nbins
                )
            else:
        ...
        ypdf, xpdfleft, patches = plt.hist(
            data,
            bins_input,
            histtype=histtype,
            color=hist_color_fm,
            label=hist_label,
            weights=data_weights / np.nansum(data_weights),
            density=False,
            lw=2,
            rwidth=0.8,
        )
        ...
        # hist_color_fm_error = (
        #     hist_color[0] / hist_color_fm_error_factor / 255,
        #     hist_color[1] / hist_color_fm_error_factor / 255,
        #     hist_color[2] / hist_color_fm_error_factor / 255
        # )
        ...
        # serrbins = plt.errorbar(
        #     xpdf_shifted,
        #     ypdf,
        #     yerr=np.array([poisson_neg, poisson_pos]),
        #     marker=".",
        #     markersize=0,
        #     linestyle="",
        #     color=hist_color_fm_error,
        #     elinewidth=2,
        #     capsize=0,
        #     alpha=0.8,
        #     zorder=15
        ...
        # avg_min_eneg = np.interp(p1s, np.flip(
        #     ycdfneg, axis=0), np.flip(xpdf[ileft[0]], axis=0))
        ...
        #     ecolor=self.lighter(hist_color_fm, 0.3),
        #     markeredgewidth=1.5,
        #     markeredgecolor="k",
        #     capsize=capsize,
        #     capthick=capthick,
        ...
        # Return the bin input positions and the vertical position of the "average"
    ...
    def weighted_stddev(self, data, weights, frequency=False, unbiased=True):
        ...
        if unbiased is True:
            if frequency is True:
                ...
                bias_estimator = 1.0 - (np.nansum(weights ** 2)) / (
                    np.nansum(weights) ** 2
                )
        else:
            bias_estimator = 1.0
    ...
    def update_statistics(self):
        ...
        diameters = (
            2
            * np.sqrt(self.clusters_long_axis * self.clusters_short_axis)
            / pixel_scale
        )
        ...
        weights = np.maximum(
            np.ceil(attainable_masses / (coffee_cell_size / 1e3) ** 3), 1
        )
        diameters_average = np.sum(diameters * weights) / np.sum(weights)
        diameters_stddev = self.weighted_stddev(
            diameters, weights, frequency=True, unbiased=True
        )
        ...
        surfaces_stddev = self.weighted_stddev(
            surfaces, weights, frequency=True, unbiased=True
        )
        ...
        # eys_stddev = self.weighted_stddev(
        #     eys, weights, frequency=True, unbiased=True)*100
    ...
    def erase_clusters(self, event):
        ...
        if self.nclusters is None:
            ...
            # Update the user interface status
            self.status_var.set(
                "Coffee Particles not Detected Yet... "
                "Use Launch Particle Detection Button..."
            )
        ...
        if self.display_type.get() != outlines_image_display_name:
            ...
            # Update the user interface status
            self.status_var.set(
                "Please select 'Display Type' = 'Cluster Outlines' to use "
                "the 'Erase Clusters' tool."
            )
        ...
        if self.erase_clusters_mode is False:
            ...
            # Update the user interface status
            self.status_var.set(
                "Entered 'Erase Clusters' mode. Click with the mouse to "
                "erase all clusters within the circle. Zoom in or out to "
                "set precision. Hit Escape or the 'Erase Clusters' button "
                "to end."
            )
    ...
    def create_histogram(self, event):
        ...
        if self.pixel_scale_var.get() == "None":
            ...
            # Update the user interface status
            self.status_var.set(
                "The Pixel Scale Has Not Been Defined Yet... Use the Left"
                " Mouse Button to Draw a Line on a Reference Object and "
                "Choose a Reference Length in mm..."
            )
        ...
        if self.comparison.nclusters is not None:
            self.psd_hist_from_data(
                self.comparison,
                hist_color=[74, 124, 179],
                hist_label=self.comparison_data_label_var.get(),
                bins_input=bins_input,
                histtype="step",
                ypos_errorbar=ypos_errorbar * 2,
                xerr_shift=-xerr_shift,
            )
    ...
    def load_data(self, event):
        ...
        csv_data_filename = filedialog.askopenfilename(
            initialdir=self.output_dir,
    ...
    def save_data(self, event):
        ...
        if self.nclusters is None:
            ...
            # Update the user interface status
            self.status_var.set(
                "Particles not Detected Yet... Use Launch Particle Detection Button..."
            )
        ...
        dataframe = pd.DataFrame(
            {
                "SURFACE": self.clusters_surface,
                "ROUNDNESS": self.clusters_roundness,
                "SHORT_AXIS": self.clusters_short_axis,
                "LONG_AXIS": self.clusters_long_axis,
                "VOLUME": self.clusters_volume,
        ...
        if self.expert_mode is True:
            ...
            full_filename = filedialog.asksaveasfilename(
                ...
                title="Select an output file name",
            )

        # Create a Pandas dataframe for stats
        stats_dataframe = pd.DataFrame(
            {
                "AVG_DIAM": [float(self.diam_average_var.get())],
                "STD_DIAM": [float(self.diam_stddev_var.get())],
                "AVG_SURF": [float(self.surf_average_var.get())],
                "STD_SURF": [float(self.surf_stddev_var.get())],
                "EFF": [float(self.eff_var.get())],
                "QUAL": [float(self.q_var.get())],
            }
        )
